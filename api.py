    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(traceback.format_exception(None, e, e.__traceback__), file=sys.stderr, flush=True)
                logger_1.error({"input": img_b64, "tunght5": "thanos", "stacktrace": str(traceback.format_exception(None, e, e.__traceback__))})
                return jsonify(data),204
        except Exception as e: 
            print(traceback.format_exception(None, e, e.__traceback__), file=sys.stderr, flush=True)
            logger_1.error({"input": dataDict, "tunght5": "thanos", "stacktrace": str(traceback.format_exception(None, e, e.__traceback__))})
            return jsonify(data),406
    except Exception as e:
        print(traceback.format_exception(None, e, e.__traceback__), file=sys.stderr, flush=True)
        logger_1.error({"tunght5": "thanos", "stacktrace": str(traceback.format_exception(None, e, e.__traceback__))})
